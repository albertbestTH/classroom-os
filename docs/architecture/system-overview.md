...
External teacher UAT uses a separate HTTPS Web/API deployment, managed PostgreSQL 16+ database, and Expo internal APK. The deterministic fixture is tagged `UAT-CLASSROOM-OS`; guarded seed/reset/verify commands refuse localhost and require explicit staging confirmation. Local filesystem profile uploads remain unsuitable for multi-instance hosting and require an approved object-storage adapter before Production.
