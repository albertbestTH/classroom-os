...
Operational teacher data uses a centralized TanStack Query policy. Today, timetable, assignments, sessions, attendance, timeline, coverages, and quick scores use a short 10-second freshness window and targeted workflow invalidation; profile and school metadata retain a longer window. React Native `AppState` drives Query focus refresh when the app becomes active, repeated events are deduplicated, and the online manager delays network refetch until connectivity returns. Mutations never retry automatically.
