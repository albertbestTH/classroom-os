...
export function RegisterForm() {
  ...
  return <div className="mt-8 space-y-6">
    <form onSubmit={register} className="grid gap-4 sm:grid-cols-2">
      <label className="text-sm font-semibold sm:col-span-2">ชื่อโรงเรียน<input className={inputClass} name="schoolName" required /></label>
      <label className="text-sm font-semibold">รหัสโรงเรียน<input className={inputClass} name="schoolCode" pattern="[A-Za-z0-9_-]+" required /></label>
      ...
      <button type="submit" className="min-h-11 rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white sm:col-span-2">สร้างโรงเรียนใหม่</button>
    </form>
