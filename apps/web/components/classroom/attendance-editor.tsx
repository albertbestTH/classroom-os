    ...
    <div className="mt-8 space-y-4">
      ...
      {initial.students.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
          <p className="font-semibold">ยังไม่มีนักเรียนในห้องนี้</p>
          <p className="mt-1 text-sm text-[#6B7280]">เพิ่มการลงทะเบียนนักเรียนในภาคเรียนนี้ก่อนเช็กชื่อ</p>
        </div>
      ) : (
        <div>
          <ul className="space-y-3">
            {initial.students.map((student) => (
              <li key={student.studentId} className="rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold">{student.firstName} {student.lastName}{student.preferredName ? ` (${student.preferredName})` : ""}</p>
                    <p className="mt-1 font-mono text-xs text-[#6B7280]">{student.studentNumber}</p>
                  </div>
                  <fieldset disabled={readOnly} className="grid grid-cols-4 gap-2">
                    <legend className="sr-only">สถานะของ {student.firstName} {student.lastName}</legend>
                    {statuses.map((status) => (
                      <label key={status.value} className="cursor-pointer">
                        <input type="radio" className="peer sr-only" name={`status-${student.studentId}`} value={status.value} checked={values[student.studentId] === status.value} onChange={() => { setValues((current) => ({ ...current, [student.studentId]: status.value })); setFeedback(null); }} />
                        <span className={`inline-flex min-h-11 min-w-14 items-center justify-center rounded-xl border border-slate-200 px-3 text-sm font-semibold ${status.style} peer-focus-visible:ring-2 peer-focus-visible:ring-blue-600 peer-focus-visible:ring-offset-2`}>{status.label}</span>
                      </label>
                    ))}
                  </fieldset>
                  {canCorrect && initial.status === "completed" && student.status && student.recordUpdatedAt ? <button type="button" onClick={() => setCorrection({ student, status: student.status!, note: student.note ?? "", reason: "" })} className="min-h-11 rounded-xl border border-blue-300 px-4 text-sm font-bold text-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600">แก้ไข</button> : null}
                </div>
                {student.corrections.length > 0 ? <details className="mt-4 border-t border-slate-100 pt-3"><summary className="cursor-pointer text-sm font-semibold text-blue-700">ประวัติการแก้ไข {student.corrections.length} ครั้ง</summary><ol className="mt-3 space-y-2">{student.corrections.map((item) => <li key={item.id} className="rounded-xl bg-slate-50 p-3 text-sm"><p><strong>{statusLabelsForCorrection(item.beforeStatus)}</strong> → <strong>{statusLabelsForCorrection(item.afterStatus)}</strong> · {item.actorName ?? "ผู้ใช้งานที่ถูกลบ"}</p><p className="mt-1 text-slate-600">เหตุผล: {item.reason}</p><time className="mt-1 block text-xs text-slate-500" dateTime={item.createdAt}>{new Intl.DateTimeFormat("th-TH", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.createdAt))}</time></li>)}</ol></details> : null}
              </li>
            ))}
          </ul>
