...
import { formatElapsedSessionTime, sessionStartWindow } from "@/lib/session-time-policy";
...
export function LiveSessionControls({ session, role, attendance }: { session: ClassSessionResult; role: UserRole; attendance: AttendanceSummary }) {
  ...
  useEffect(() => {
    ...
    const delay = Math.max(0, new Date(session.scheduledEnd).getTime() - Date.now()) + 100;
    const timer = window.setTimeout(() => router.refresh(), delay);
    return () => window.clearTimeout(timer);
  }, [router, session.scheduledEnd, session.status]);

  const startWindow = sessionStartWindow(session.scheduledStart, session.scheduledEnd, now);

  async function start() {
    if (startWindow !== "available") return;
  ...
  return (
    <div className="space-y-4">
      ...
      {session.status === "live" ? <div className="rounded-2xl bg-blue-600 p-6 text-center text-white"><p className="text-sm font-semibold text-blue-100">เวลาที่สอนไปแล้ว</p><p className="mt-2 font-mono text-5xl font-bold tabular-nums" aria-live="off">{formatElapsedSessionTime(session.startedAt, session.scheduledStart, now)}</p></div> : null}
      {session.status === "scheduled" ? <button type="button" onClick={start} disabled={pending || startWindow !== "available"} className="min-h-14 w-full rounded-xl bg-blue-600 px-6 text-lg font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60">{pending ? "กำลังเริ่ม…" : startWindow === "too-early" ? "ยังไม่ถึงเวลาเริ่มคาบ" : startWindow === "expired" ? "เลยเวลาเริ่มคาบแล้ว" : "เริ่มคาบเรียน"}</button> : null}
      {session.status === "live" ? <div className="grid gap-3 sm:grid-cols-3"><Link aria-label={`${attendanceActionLabel(attendance, session.status)} บันทึกแล้ว ${attendance.recorded} จาก ${attendance.enrolled} คน`} href={`/sessions/${session.id}/attendance?classroomId=${session.classroomId}`} className="inline-flex min-h-14 items-center justify-center rounded-xl bg-blue-600 px-5 text-base font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2">{attendanceActionLabel(attendance, session.status)}</Link><Link href={`/sessions/${session.id}/scores`} className="inline-flex min-h-14 items-center justify-center rounded-xl border border-blue-200 bg-blue-50 px-5 text-base font-bold text-blue-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2">คะแนนด่วน</Link><button type="button" onClick={() => setConfirmEnd(true)} disabled={pending} className="min-h-14 rounded-xl border border-red-300 bg-white px-5 text-base font-bold text-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60">จบคาบเรียน</button></div> : null}
