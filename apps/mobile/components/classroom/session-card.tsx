...
export function SessionCard({ item, primary = false, canMutate = true, onStart }: Props) {
  ...
  return <Card style={primary ? { backgroundColor: colors.primary, borderColor: colors.primary } : undefined}>
    ...
    {action ? <AppButton label={action} tone={primary ? "inverse" : "secondary"} disabled={item.status === "scheduled" && !canMutate} onPress={() => { if (item.status === "scheduled") onStart?.(item); else if (item.session) router.push(item.status === "completed" ? `/sessions/${item.session.id}/summary` : `/sessions/${item.session.id}`); }} /> : null}
  </Card>;
}

const styles = StyleSheet.create({ row: { flexDirection: "row", alignItems: "flex-start", gap: spacing.md }, flex: { flex: 1 }, classroom: { fontSize: 22, fontWeight: "900" }, subject: { marginTop: 4, lineHeight: 21 } });
