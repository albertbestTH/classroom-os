...
export function ProgressBar({ label, value, max, tone = "primary" }: { label: string; value: number; max: number; tone?: "primary" | "success" | "warning" }) {
  ...
  return <View accessibilityRole="progressbar" accessibilityLabel={`${label} ${value} จาก ${max}`} accessibilityValue={{ min: 0, max, now: value }} style={styles.progressGroup}>
    <View style={styles.row}><Text style={styles.progressLabel}>{label}</Text><Text style={styles.progressValue}>{percentage}%</Text></View>
    <View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${percentage}%` }, tone === "success" && styles.progressSuccess, tone === "warning" && styles.progressWarning]} /></View>
    <Text style={styles.muted}>{value} จาก {max}</Text>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  progressGroup: { gap: spacing.sm }, progressLabel: { color: colors.text, fontSize: 16, fontWeight: "700" }, progressValue: { color: colors.primaryDark, fontSize: 16, fontWeight: "800" }, progressTrack: { height: 10, overflow: "hidden", borderRadius: radius.pill, backgroundColor: colors.border }, progressFill: { height: "100%", borderRadius: radius.pill, backgroundColor: colors.primary }, progressSuccess: { backgroundColor: colors.success }, progressWarning: { backgroundColor: colors.warning },
});
