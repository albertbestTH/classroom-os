...
import { AppButton, AppHeader, Card, EmptyState, ErrorState, LoadingSkeleton, MetricCard, SafeScreen, SectionHeader, StatusBadge } from "@/components/ui/primitives";
...
export default function RecordsScreen() {
  ...
  return <SafeScreen>
    ...
    {live?.session ? <Card>
      <Text style={styles.title}>{live.timetableEntry.classroomName}</Text>
      <Text style={styles.subtitle}>{live.timetableEntry.subjectName} · นักเรียน {live.session.enrolledStudentCount} คน</Text>
    ...
    {completed.length ? completed.map((item) => <Card key={item.session!.id}>
      <Text style={styles.title}>{item.timetableEntry.classroomName}</Text>
      <Text style={styles.subtitle}>{item.timetableEntry.subjectName} · เช็กชื่อ {item.session!.attendanceRecordedCount}/{item.session!.enrolledStudentCount}</Text>
      <AppButton label="ดูสรุปคาบ" tone="secondary" onPress={() => router.push(`/sessions/${item.session!.id}/summary`)} />
    </Card>) : <Text style={styles.muted}>เมื่อจบคาบ รายการสรุปจะปรากฏที่นี่</Text>}
    <Card>
      <Text style={styles.title}>คะแนนด่วน</Text>
      <Text style={styles.muted}>ขั้นถัดไปจะรองรับคะแนนการมีส่วนร่วม แบบทดสอบสั้น และงานในคาบ โดยไม่แทน Gradebook เต็มรูปแบบบนเว็บ</Text>
    </Card>
  </SafeScreen>;
}
...
const styles = StyleSheet.create({
  ...
  subtitle: { color: colors.text, fontSize: 15, lineHeight: 22 },
